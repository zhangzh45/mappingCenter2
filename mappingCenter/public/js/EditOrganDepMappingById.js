...
$(document).ready(function() {
    oTable=$("#EditOrganDepTable").DataTable({
        "aoColumnDefs":
            [
    ...
    $('#EditOrganDepTable tbody').on( 'click', 'td.details-control',function () {
        ...
        if (row.child.isShown()) {
            ...
            // Open this row
            var id = this.dataset.id;   // 业务角色id
            var name = this.dataset.value;   // 业务角色名称
            var list = document.getElementById(id).value;  // 该业务角色与职位的映射关系集
            var organDepList = JSON.parse(list);
            row.child(format(organDepList,name,id)).show();
            tr.addClass('shown');
        }
    } );
} );
...
function format (d,name,id) {
    ...
    if ( d!=null && d.length > 0 ) {
        for (var i=0;i< d.length;i++ ) {
            if ( d[i].isMapped  ) {
                sOut += '<tr><td>'+ d[i].posName+'</td><td>'+ d[i].roleName+'</td><td><input  name="subCheck" type="checkbox" checked id="'+ subid +'" value="'+ id +'" data-name="'+ name +'" data-id="'+ d[i]._id+'" data-value="'+ d[i].posName+'" onclick="subCheckClick(\''+ subid+'\',\''+ id+'\',\''+ d[i]._id+'\')" /></td></tr>';
            } else  {
                sOut += '<tr><td>'+  d[i].posName+'</td><td>'+ d[i].roleName+'</td><td><input name="subCheck" type="checkbox"  id="'+ subid +'"  value="'+ id +'" data-name="'+ name +'" data-id="'+ d[i]._id+'" data-value="'+ d[i].posName+'"  onclick="subCheckClick(\''+ subid+'\',\''+ id+'\',\''+ d[i]._id+'\')" /></td></tr>';
            }
        }
    }
    sOut += '</table>';
...
function submitForm() {
    ...
    $('#EditOrganDepTable tbody td.details-control').click();
    ...
    if (checknum > 0) {
        ...
        $("#EditOrganDepForm").attr("action", '/rolemap/updateOrganDepMapping/' + mapId + '&'+role2PosOldString+ '&' + role2PosNewString);
