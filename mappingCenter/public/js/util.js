...
Util.loading = function (b) {
    var c = {content: "loading...", show: 1000, delay: 0, model: false};
...
(function (c) {
        ...
        if (f == "open") {
            if (d == 0) {
                ...
                c(window).bind("resize.mask", function () {
                    e.css({width: c(window).width() + "px", height: c(window).height() + "px"})
                })
            }
            d++
        } else {
    ...
    c.fn.dialog = function (g) {
        ...
        if (typeof g == "string") {
            ...
            var i = {fixed: true, closable: true, mask: true};
            g = c.extend(i, g);
            if (!g) {
                g = {}
            }
            ...
            var h = f.outerWidth();
            var e = f.outerHeight();
            c(window).bind("resize.dialog", function () {
                var m = 0;
                if (g.fixed) {
            ...
            f.find(".dialog_title").draggable({target: f})
        }
        return f
    };
    ...
    c.fn.popMenu = function (e) {
        ...
        if (f.autoClose) {
            if (f.closeAfterClick == false) {
                k.unbind("mouseup.popmenu").bind("mouseup.popmenu", function (l) {
                    l.stopPropagation()
                })
            }
        ...
        if (f.autoPosition && g + f.offsetY + k.outerHeight() > c(window).height() + c(document).scrollTop()) {
            k.css({top: c(window).height() - k.outerHeight() + c(document).scrollTop(), left: h + f.offsetX})
        } else {
            k.css({top: g + f.offsetY, left: h + f.offsetX})
        }
    };
    c.fn.spread = function (j) {
        ...
        var A = parseInt(e.css("padding-left").replace("px", "")), v = parseInt(e.css("padding-right").replace("px", "")), t = parseInt(e.css("padding-top").replace("px", "")), f = parseInt(e.css("padding-bottom").replace("px", "")), u = parseInt(C.css("padding-left").replace("px", "")), o = parseInt(C.css("padding-right").replace("px", "")), n = parseInt(C.css("padding-top").replace("px", "")), B = parseInt(C.css("padding-bottom").replace("px", ""));
        var l = {w: e.width() + A + v, h: e.height() + t + f, ml: 10, mt: 10, maxWidth: C.width(), s: 150};
        var g = c.extend(l, j);
        var m = g.w, x = g.h, y = g.ml, r = g.mt, z = g.maxWidth, p = g.s;
        c.each(q, function (s, w) {
            var h = c(w);
            h.css({top: "-" + x + "px", left: 0})
        });
    ...
    c.fn.draggable = function (e) {
        ...
        c(this).unbind("mousedown.drag").bind("mousedown.drag", function (i) {
            ...
            c(document).bind("mousemove.drag", function (p) {
                var o = p.pageX - l + k;
                var n = p.pageY - j + h;
                if (f.bounding) {
    ...
    c.fn.suggest = function (f) {
        var j = c(this);
        var i = {
            valueField: "value", width: j.outerWidth(), format: function (l) {
                return l.text
            }
        };
        var g = c.extend(i, f);
        ...
        var e = -1;
        var h = "";
        j.unbind("keydown.suggest").bind("keydown.suggest", function (m) {
        ...
        }).unbind("keyup.suggest").bind("keyup.suggest", function (m) {
            var n = j.data("suggest");
            var l = j.val();
            if (l == "") {
                n.empty().popMenu("close")
                ...
                if (l != h) {
                    ...
                    c.get(g.url, {q: l}, function (r) {
                        ...
                        if (o.length == 0) {
                            ...
                            n.popMenu({target: j, zindex: 4});
                            n.find(".suggest_item").bind("mousedown", function (t) {
                                t.preventDefault();
            ...
            h = l
        }).unbind("blur.suggest").bind("blur.suggest", function (l) {
    ...
    c.fn.switchbutton = function (f) {
        ...
        var g = c.extend(l, f);
        var k = c(this), e = k.find(".switch"), i = k.find(".switch-left"), h = k.find(".switch-right");
        k.css({width: g.width, height: g.height, "line-height": g.height - 10 + "px"});
        e.css({width: g.innerWidth, height: g.innerHeight, background: g.innerBackground});
        k.on("click", function () {
            if (!g.enable) {
                return
            }
    ...
    c.fn.streamInput = function (u) {
        ...
        this.attr({contentEditable: "true", spellcheck: "false", accesskey: "q", stream_id: r.stream_id});
        c(f.face).attr("for_stream", r.stream_id);
        b[r.stream_id] = r, a = r;
        var o = false, i = false, k = 0, l = 0, m = 0, p = !!f.home;
        c(f.target).off("click.stream keyup.stream").on("click.stream keyup.stream", function (w) {
            a = b[c(this).attr("stream_id")];
            a.target = c(this);
            var v = window.getSelection();
            a.range = v.getRangeAt(0).cloneRange()
        });
        c(f.target).off("DOMSubtreeModified.stream").on("DOMSubtreeModified.stream", function (D) {
            if (!o) {
                return
            }
            o = false;
            ...
            if (i) {
                ...
                B.deleteContents();
                var w = c("<div>" + E + "</div>");
                var A = w.html(), y = document.createElement("div"), F = document.createDocumentFragment(), z, v;
                if (A) {
                    y.innerHTML = A;
        ...
        c(f.target).off("paste.stream").on("paste.stream", function (A) {
            A = A.originalEvent;
        ...
        c(f.face).off("click.stream").on("click.stream", function () {
            ...
            c("#faces_dialog").popMenu({target: this, fixed: p, offsetY: 10, zindex: 999})
        });
        c("#faces_dialog ul li").off("click.stream").on("click.stream", function (z) {
        ...
        function n(w, x, B, D) {
            var A = w.html(), y = document.createElement("div"), C = document.createDocumentFragment(), z, v;
            if (A) {
                y.innerHTML = A;
                while ((z = y.firstChild)) {
                    v = C.appendChild(z)
